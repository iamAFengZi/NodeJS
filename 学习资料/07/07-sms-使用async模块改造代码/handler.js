...
// 处理显示学员列表
module.exports.get.students = function (req, res, next) {
  // 1. 获取 students 集合中的所有数据
  db.findAll('students', function (err, docs) {
    if (err) {
      // throw err;
      // 当调用 next() 函数的时候，如果中间传递了错误对象，那么会自动跳转到错误处理中间件
      next(err);
      return;
    }
...
module.exports.get.edit = function (req, res) {
  ...
  async.parallel({
    ...
    if (err) {
     ...
     // 6. 渲染(// 5. 让选中某个城市和专业)
     res.render('edit', {item: result.stu, majors: result.major, cities: result.city});
  });
